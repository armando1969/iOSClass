...
class DetailViewController: UIViewController {
    ...
    var favoriteMovie = FavoriteMovie(id: 0, originalTitle: "", overview: "", posterPath: "", favoriteIndex: -1)
    var selector = 0
    ...
    private func setupUI() {
        ...
        if selector == 0 {
            ...
            originalTitleLabel.text = favoriteMovie.originalTitle
            overviewLabel.text = favoriteMovie.overview
            viewModel.getImageData(favoriteMovie.posterPath) { data in
                DispatchQueue.global().async {
                    if let data = data {
                        let image = UIImage(data: data)
                        DispatchQueue.main.async {
